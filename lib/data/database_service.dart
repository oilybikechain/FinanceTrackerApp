...
class DatabaseService {
  ...
  static Database?
  _database; // Cache the database instance
  ...
  Future<int> insertAccount(
    ...
    try {
      return await db.insert(
        'accounts',
        account.toMap()..remove(
          'id',
        ), // Use toMap, remove ID for auto-increment
        conflictAlgorithm:
            ConflictAlgorithm
  ...
  Future<int> updateAccount(
    ...
    return await db.update(
      'accounts',
      account
          .toMap(), // Use toMap to get all fields
      where: 'id = ?',
      whereArgs: [account.id],
    );
  }

  Future<void> updateAccountSortOrder(
  ...
  Future<int> insertTransaction(
    ...
    try {
      ...
      print(
        "Transaction data: ${transaction.toMap()}",
      );
      rethrow;
    }
  }
  ...
  getTransactions({
    ...
    int?
    limit, // Optional limit for pagination/recent items
  }) async {
    Database db = await database;
    ...
    maps = await db.query(
      ...
      whereArgs:
          whereArgs.isNotEmpty
              ...
              : null,
      orderBy:
          'timestamp DESC', // Most recent first by default
      limit: limit,
    );

    return List.generate(
      maps.length,
      (i) => Transactions.fromMap(maps[i]),
    );
  }

  Future<Transactions?> getTransactionById(
    int id,
  ) async {
    Database db = await database;
    final List<Map<String, dynamic>> maps =
    ...
    return maps.isNotEmpty
        ? Transactions.fromMap(maps.first)
        : null;
  }
   ...
   Future<double> getAccountBalance(int accountId) async {
     ...
     try {
       ...
       if (accData.isNotEmpty) {
           initialBalance = (accData.first['initial_balance'] as num?)?.toDouble() ?? 0.0;
       } else {
         // Handle case where account ID might be invalid (optional)
         print("Warning: Account ID $accountId not found for balance calculation.");
  ...
  Future<int> insertRecurringTransaction(
    ...
    try {
      ...
      print(
        "Error inserting recurring transaction: $e",
      );
      print(
        "Recurring data: ${recurring.toMap()}",
      );
      rethrow;
    }
  }
  ...
  getAllRecurringTransactions() async {
    ...
    return List.generate(
      maps.length,
      (i) => RecurringTransaction.fromMap(
        maps[i],
      ),
    );
  }
  ...
  Future<void> close() async {
    ...
    await db.close();
    _database =
        null; // Ensure re-initialization next time
